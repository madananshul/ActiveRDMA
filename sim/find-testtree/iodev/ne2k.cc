...
void bx_ne2k_c::rdma_packet_sender(void *p, void *data, int len)
{
    bx_ne2k_c *t = (bx_ne2k_c *)p;
    t->ethdev->sendpkt(data, len);
}
...
void bx_ne2k_c::rx_handler(void *arg, const void *buf, unsigned len)
{
  ...
  if (class_ptr->rdma->handle_packet((void *)buf, len)) return;
